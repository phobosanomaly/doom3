...
bool Sys_GetPath(sysPath_t type, idStr &path) {
	...
	case PATH_CONFIG:
		...
		if (s)
			idStr::snPrintf(buf, sizeof(buf), "%s/doom3", s);
		else
			idStr::snPrintf(buf, sizeof(buf), "%s/.config/doom3", getenv("HOME"));
	...
	case PATH_SAVE:
		...
		if (s)
			...
			idStr::snPrintf(buf, sizeof(buf), "%s/.local/share/doom3", getenv("HOME"));
